...
const TARGET_SHOTS: ShotInfo[] = [
  {
    ...
    tagColor: 'border-rose-500/30 text-rose-600 dark:text-rose-400 bg-rose-500/10'
  },
  {
    name: 'CLEAR',
    description: 'High, deep trajectory sending the shuttle deep to the opponent rear court.',
    tagColor: 'border-sky-500/30 text-sky-600 dark:text-sky-400 bg-sky-500/10'
  },
  {
    name: 'DROP',
    description: 'Deceptive downward trajectory softly placed just over the net tape.',
  ...
  },
  {
    ...
    tagColor: 'border-purple-500/30 text-purple-600 dark:text-purple-400 bg-purple-500/10'
  }
];
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs uppercase font-mono tracking-widest text-[var(--text-secondary)] font-semibold">
          Badminton Shot Taxonomy (5 Target Classes)
        </h3>
        <span className="text-xs text-[var(--text-muted)] font-mono">
          Standard Dataset Classes
        </span>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {TARGET_SHOTS.map((shot) => (
          <div
            key={shot.name}
            className={`p-3.5 rounded-xl border ${shot.tagColor} transition-all duration-200 hover:scale-[1.02] shadow-sm`}
          >
            <div className="flex items-center justify-between mb-1.5">
              <span className="font-bold text-sm tracking-wide">{shot.name}</span>
              <span className="text-[10px] uppercase font-mono opacity-80 px-1.5 py-0.5 rounded bg-black/5 dark:bg-white/10">
                Class
              </span>
            </div>
